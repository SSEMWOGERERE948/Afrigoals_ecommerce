...
Generate insights in the required JSON format.`,
    ...
    try {
      ...
      insights = {
        salesTrends: {
          summary: `Revenue this week: ugx${currentRevenue.toFixed(2)} (${revenueChange > 0 ? "+" : ""}${revenueChange.toFixed(1)}% vs last week)`,
          highlights: [
            `${revenuePeriod.currentOrderCount || 0} orders this week`,
            `Average order value: ugx${avgOrderValue.toFixed(2)}`,
            topProducts[0]
